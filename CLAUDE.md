...
- Modal galeri: global `openSubjectGallery(subjectKey)` membaca `fileDatabase` yang memetakan kunci subjek (NusaBiology, NusaFisika, NusaChem, NusaMath, NusaPeriodic) ke nama file di `assets/<subjectKey>/`. **Menambah gambar galeri = menaruh file `.webp` di folder yang sesuai DAN menambahkan namanya ke `fileDatabase`.**
